...
{
public:
    SFEVL53L1X(TwoWire &i2cPort = Wire, int shutdownPin = -1,
               int interruptPin =
                   -1); // Constructs our Distance sensor without an interrupt or shutdown pin
    ...
    void setIntermeasurementPeriod(uint16_t intermeasurement); // Set time between measurements in
                                                               // ms
    ...
    uint16_t getSignalPerSpad(); // Returns the average signal rate per SPAD (The sensitive pads
                                 // that detect light, the VL53L1X has a 16x16 array of these) in
                                 // kcps/SPAD, or kilo counts per second per SPAD.
    uint16_t
    getAmbientPerSpad();      // Returns the ambient noise when not measuring a signal in kcps/SPAD.
    ...
    uint8_t getRangeStatus(); // Returns the range status, which can be any of the following. 0 = no
                              // error, 1 = signal fail, 2 = sigma fail, 7 = wrapped target fail
    void setOffset(int16_t offset); // Manually set an offset in mm
    int16_t getOffset();            // Get the current offset in mm
    void setXTalk(
        uint16_t xTalk); // Manually set the value of crosstalk in counts per second (cps), which is
                         // interference from any sort of window in front of your sensor.
    uint16_t getXTalk(); // Returns the current crosstalk value in cps.
    void setDistanceThreshold(
        uint16_t lowThresh, uint16_t hiThresh,
        uint8_t window); // Set bounds for the interrupt. lowThresh and hiThresh are the bounds of
                         // your interrupt while window decides when the interrupt should fire. The
                         // options for window are shown below.
    ...
    *
    * To set the center, set the pad that is to the right and above the exact center of the region
    you'd like to measure as your opticalCenter*/
    void setROI(uint8_t x, uint8_t y,
                uint8_t opticalCenter); // Set the height and width of the ROI(region of interest)
                                        // in SPADs, lowest possible option is 4. Set optical center
    ...
    void
    setSignalThreshold(uint16_t signalThreshold); // Programs the necessary threshold to trigger a
                                                  // measurement. Default is 1024 kcps.
    uint16_t getSignalThreshold(); // Returns the signal threshold in kcps
    void setSigmaThreshold(
        uint16_t sigmaThreshold);  // Programs a new sigma threshold in mm. (default=15 mm)
    uint16_t getSigmaThreshold();  // Returns the current sigma threshold.
    void startTemperatureUpdate(); // Recalibrates the sensor for temperature changes. Run this any
                                   // time the temperature has changed by more than 8°C
    void
    calibrateOffset(uint16_t targetDistanceInMm); // Autocalibrate the offset by placing a target a
                                                  // known distance away from the sensor and passing
                                                  // this known distance into the function.
    void
    calibrateXTalk(uint16_t targetDistanceInMm); // Autocalibrate the crosstalk by placing a target
                                                 // a known distance away from the sensor and passing
                                                 // this known distance into the function.
private:
    TwoWire *_i2cPort;
    int _shutdownPin;
